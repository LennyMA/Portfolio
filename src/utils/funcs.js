 ...
 */
export const getCurrentCmdArry = (history) => _.split(history[0].trim(), " ");
...
export const checkRedirect = (rerender, currentCommand, command) =>
  ...
  currentCommand.length < 4 && // if the number of arg is valid (not `projects go 1 sth`)
...
export const checkThemeSwitch = (rerender, currentCommand, themes) =>
  ...
  currentCommand.length > 1 && // current command has arg
  currentCommand.length < 4 && // if the number of arg is valid (not `themes set light sth`)
...
export const argTab = (inputVal, setInputVal, setHints, hintsCmds) => {
  ...
  // 7) if input is 'socials go '
    ...
    ["1.Github", "2.Facebook", "3.Instagram", "4.Twitter"].forEach((t) => {
      hintsCmds = [...hintsCmds, t];
    });
    return hintsCmds;
  }
